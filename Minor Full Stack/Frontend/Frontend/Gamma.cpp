...
// Function to increase shadows
void increaseShadows(vector<Pixel> &image, int width, int height, int shadowValue)
{
    for (auto &pixel : image)
    {
        pixel.r = static_cast<unsigned char>(clamp(pixel.r - shadowValue, 0, 255));
        pixel.g = static_cast<unsigned char>(clamp(pixel.g - shadowValue, 0, 255));
        pixel.b = static_cast<unsigned char>(clamp(pixel.b - shadowValue, 0, 255));
    }
}
...
int main()
{
    ...
    int shadowValue = 3; // Increase shadows
    // Contrast adjustment (manually set, not asked from user)
    float contrastFactor = 1.2f; // Reduce contrast
    int brightnessValue;
    // do
    // {
    //     int choice;
    //     cout << "Do you wish to enhnace the image further? (1 -- Yes \n 2--No) ";
    //     cin >> choice;
    //     switch (choice)
    //     {
    //     case 1:
    ...
    //         cout << "Enter the value for increasing brightness (0-70): ";
    //         cin >> brightnessValue;
    //         adjustBrightness(gammaImage, width, height, brightnessValue);

    //         adjustContrast(gammaImage, width, height, contrastFactor);

    //         // Shadow enhancement (manually set, not asked from user)

    //         // increaseShadows(gammaImage, width, height, shadowValue);

    //         // Apply Gaussian smoothing
    //         // applyGaussianSmoothing(gammaImage, width, height, 3, 1.0);

    //         // Save the enhanced image
    //         if (!writePPM("enhanced_image.ppm", gammaImage, width, height))
    //         {
    //             cerr << "Error saving enhanced image." << endl;
    //             return -1;
    //         }
    //         cout << "Enhanced image saved as enhanced_image.ppm" << endl;
    //         break;
